...
rm -f iMolpro.spec
cat > _pyinstaller_entry.py << 'PYEOF'
# Generated by build.sh: PyInstaller's entry-point script must not share a
# name with a package it imports, or the package gets silently dropped from
# the bundle (PyInstaller indexes bundled modules by name, and an 'iMolpro'
# entry-point script collides with the 'iMolpro' package). --name iMolpro
# keeps the built executable/app itself named iMolpro as before.
