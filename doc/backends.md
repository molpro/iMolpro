...
- `run_command` The command used to run the job. This might simply be `molpro`, which is the default, or an explicit filesystem path to source a particular instance of molpro. The command can be appended with some options. At job submission time, the input file name is appended to `run_command`. Care should be taken that the default environment on the remote machine provides any runtime libraries that Molpro might need, and if necessary an approach to deal with this aspect is to create a script on the remote that sets the environment then launches `molpro` with the arguments provided to the script. This consideration is not normally necessary with the official distributed molpro binary.
